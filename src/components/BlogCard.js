...
export default function BlogCard({ blog }) {
  ...
  const [isLikedByUser, setIsLikedByUser] = useState(
    blog.likers.includes(currentUser.uid)
  );
        ...
        <CardActions sx={{ display: "flex", justifyContent: "space-between" }}>
          <Box>
            <Button
              size="small"
              onClick={() =>
                navigate(`/details/${newBlog?.id}`, { state: newBlog })
              }
            >
              Details
            </Button>
            {newBlog?.uid === currentUser.uid && (
              <Button
                size="small"
                onClick={() => navigate(`/updateblog`, { state: newBlog })}
              >
                Edit
              </Button>
            )}
          ...
          >
            <Button size="small" onClick={handleLike}>
              <FavoriteIcon
                sx={{
                  color: isLikedByUser ? "red" : "lightgrey",
                  width: "1.5rem",
                }}
              />{" "}
            </Button>
            ...
            <Button size="small">
              <ChatBubbleOutlineIcon
                ...
                sx={{
                  color: commentCount ? "green" : "lightgrey",
                  width: "1.5rem",
                }}
              />{" "}
            </Button>
